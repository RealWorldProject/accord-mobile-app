class TimeCalculator {
  ...
  static String getTimeDifference(DateTime givenTime) {
    ...
    if (seconds < 60) {
      ...
      minutes = seconds ~/ 60;
      if (minutes < 60) {
        timeDifference = "$minutes ${minutes > 1 ? "minutes" : "minute"} ago";
        //  ${remainingSeconds()} seconds ago";
      } else {
        hours = minutes ~/ 60;
        if (hours < 24) {
          timeDifference = "$hours ${hours > 1 ? "hours" : "hour"} ago";
          // ${remainingMinutes()} ${remainingMinutes() > 1 ? "minutes" : "minute"} ago";
        } else {
          days = hours ~/ 24;
          if (days < 7) {
            timeDifference = "$days ${days > 1 ? "days" : "day"} ago";
            // ${remainingHours()} ${remainingHours() > 1 ? "hours" : "hour"} ago";
          } else {
            timeDifference = dateFormatter(givenTime: givenTime);
            // weeks = days ~/ 7;
            // if (weeks < 4 && days < 31) {
            //   timeDifference =
